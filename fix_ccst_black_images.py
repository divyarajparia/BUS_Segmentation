#!/usr/bin/env python3
"""
Fix CCST Black Images Issue
===========================
Patch the CCST implementation to properly normalize decoder output
"""
...
def fixed_tensor_to_pil(tensor):
    """Fixed tensor to PIL conversion with proper normalization"""
...
def create_fixed_ccst_exact_replication():
    """Create a fixed version of CCST exact replication as a new file"""
    
    print("🔧 Creating fixed CCST implementation...")
...
'''
    ...
    fixed_file = "ccst_exact_replication_FIXED.py"
    with open(fixed_file, 'w', encoding='utf-8') as f:
        f.write(fixed_content)
    
    print(f"   ✅ Created fixed implementation: {fixed_file}")
    return fixed_file

def test_fixed_ccst():
    """Test the fixed CCST implementation"""
    print("🧪 Testing Fixed CCST Implementation")
    print("=" * 50)
    ...
    try:
        ...
        from ccst_exact_replication_FIXED import FixedCCSTStyleExtractor, fixed_tensor_to_pil_global
        
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        style_extractor = FixedCCSTStyleExtractor(device)
        ...
        if os.path.exists(test_image_path):
            ...
            # Apply style transfer
            stylized_tensor = style_extractor.apply_style_transfer(
                image_tensor, style_dict, alpha=0.8
            )
            
            print(f"   ✅ Style transfer successful!")
            print(f"   📐 Output range: [{stylized_tensor.min():.4f}, {stylized_tensor.max():.4f}]")
            
            # Convert to PIL and save
            styled_pil = fixed_tensor_to_pil_global(stylized_tensor)
            test_output = "test_fixed_ccst_output.png"
            styled_pil.save(test_output)
            
            print(f"   💾 Test image saved: {test_output}")
            print(f"   🔍 Check this image - it should NOT be black!")
...
def main():
    """Main fix function"""
    print("🚀 Fixing CCST Black Images Issue")
    print("=" * 60)
    
    # Create fixed implementation
    fixed_file = create_fixed_ccst_exact_replication()
    ...
    print("   • Decoder output normalization")
    print("   • Tensor-to-PIL conversion")
    print("   • Min-max scaling instead of naive clamping")
    print(f"\n📁 Files created:")
    print(f"   • {fixed_file} (fixed implementation)")
    print(f"   • test_fixed_ccst_output.png (test result)")
    print("\n🎯 Next steps:")
    print(f"   • Use the fixed implementation: python {fixed_file}")
    print("   • Or import: from ccst_exact_replication_FIXED import run_fixed_ccst_pipeline")
