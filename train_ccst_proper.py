#!/usr/bin/env python3
"""
Proper CCST Training Script
Uses CCST-augmented dataset and follows the IS2D infrastructure
"""
...
from dataset.BioMedicalDataset.CCSTDataset import CCSTDataset
...
class CCSTTrainer:
    ...
    def setup_datasets(self):
        """Setup CCST-augmented train dataset and BUSI val/test datasets"""
        print("Setting up CCST-augmented datasets...")
        ...
        # Create CCST training dataset (Original BUSI + CCST-augmented BUS-UCLM)
        self.train_dataset = CCSTDataset(
            busi_path=self.args.busi_path,
            ccst_path=self.args.ccst_path,
            mode='train',
            transform=self.image_transform,
            target_transform=self.target_transform
        )
        
        # Use BUSI for validation and testing (to maintain consistency)
        ...
        print(f"Train samples (CCST-augmented): {len(self.train_dataset)}")
        print(f"Val samples (BUSI): {len(self.val_dataset)}")
        print(f"Test samples (BUSI): {len(self.test_dataset)}")
    ...
    def save_checkpoint(self, epoch, is_best=False):
        ...
        save_dir = os.path.join(self.args.save_path, "CCST", "model_weights")
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.final_epoch + 1):
            ...
            with open(os.path.join(self.args.save_path, 'ccst_training_history.json'), 'w') as f:
                json.dump(training_history, f, indent=2)
        
        print("CCST training complete!")
        
        # Test the best model
        test_results = self.test_model()
        
        # Save test results
        with open(os.path.join(self.args.save_path, 'ccst_test_results.json'), 'w') as f:
            json.dump(test_results, f, indent=2)
        
        print("CCST Test Results:")
...
def main():
    parser = argparse.ArgumentParser(description='CCST Training - Proper Implementation')
    parser.add_argument('--busi-path', type=str, default='dataset/BioMedicalDataset/BUSI',
                       help='Path to BUSI dataset')
    parser.add_argument('--ccst-path', type=str, default='ccst_output',
                       help='Path to CCST output directory')
    ...
    print("🚀 CCST Training - Proper Implementation")
    print("=" * 60)
    print(f"BUSI Dataset: {args.busi_path}")
    print(f"CCST Output: {args.ccst_path}")
    ...
    trainer = CCSTTrainer(args)
    results = trainer.train()
    
    print("\n🎉 CCST Training Complete!")
